...
def bloombergScrap(mnemo, ric):
    try:
        ...
        df = np.array(df)
        # logging.debug('got from BBG %s', df)
    except:
...
#	logging.info('number of data to be saved: %s ', len(toDB.index))
    ...
    if len(toDB.index) > 0:
        ...
        except: logging.error('error in saving quote %s %s %s', mnemo, ric, toDB)
    else: logging.info('no data to be saved for quote %s %s ', mnemo, ric)
